...
export const C_SocialQueue = {
    ...
    GetConfig: (): SocialQueueConfig => {
        return {
            ...
            QUEUE_MULTIPLIER: 0,
            PLAYER_MULTIPLIER: 0,
            PLAYER_FRIEND_VALUE: 0,
            PLAYER_GUILD_VALUE: 0,
            THROTTLE_INITIAL_THRESHOLD: 0,
            THROTTLE_DECAY_TIME: 0,
            THROTTLE_PRIORITY_SPIKE: 0,
            THROTTLE_MIN_THRESHOLD: 0,
            THROTTLE_PVP_PRIORITY_NORMAL: 0,
            THROTTLE_PVP_PRIORITY_LOW: 0,
            THROTTLE_PVP_HONOR_THRESHOLD: 0,
            THROTTLE_LFGLIST_PRIORITY_DEFAULT: 0,
            THROTTLE_LFGLIST_PRIORITY_ABOVE: 0,
            THROTTLE_LFGLIST_PRIORITY_BELOW: 0,
            THROTTLE_LFGLIST_ILVL_SCALING_ABOVE: 0,
            THROTTLE_LFGLIST_ILVL_SCALING_BELOW: 0,
            THROTTLE_RF_PRIORITY_ABOVE: 0,
            THROTTLE_RF_ILVL_SCALING_ABOVE: 0,
            THROTTLE_DF_MAX_ITEM_LEVEL: 0,
            THROTTLE_DF_BEST_PRIORITY: 0,
        };
    },
    GetGroupForPlayer: (
        playerGUID: string
    ): [groupGUID: string, isSoloQueueParty: boolean] => {
        return ["", false];
    },
    GetGroupInfo: (
        groupGUID: string
    ...
    GetGroupMembers: (groupGUID: string): LuaArray<SocialQueuePlayerInfo> => {
        return {} as any;
    },
    GetGroupQueues: (
        groupGUID: string
    ): LuaArray<SocialQueueGroupQueueInfo> => {
        return {} as any;
    },
    RequestToJoin: (
        groupGUID: string,
...
export type SocialQueueConfigUpdatedEvent = (
    frame: UIFrame,
    e: "SOCIAL_QUEUE_CONFIG_UPDATED"
) => void;
export type SocialQueueUpdateEvent = (
    frame: UIFrame,
    e: "SOCIAL_QUEUE_UPDATE",
    groupGUID: string,
    numAddedItems: number | undefined
) => void;
