...
export const C_Commentator = {
    ...
    GetCommentatorHistory: (): CommentatorHistory => {return {series: {} as any, teamDirectory: {} as any, overrideNameDirectory: {} as any}},
    GetCurrentMapID: (): number | undefined => {return 0},
    GetDampeningPercent: (): number => {return 0},
    GetDistanceBeforeForcedHorizontalConvergence: (): number => {return 0},
    GetDurationToForceHorizontalConvergence: (): number => {return 0},
    GetExcludeDistance: (): number => {return 0},
    GetHardlockWeight: (): number => {return 0},
    GetHorizontalAngleThresholdToSmooth: (): number => {return 0},
    GetIndirectSpellID: (trackedSpellID: number): number => {return 0},
    GetInstanceInfo: (mapIndex: number, instanceIndex: number): [mapID: number, mapName: string | undefined, status: number, instanceIDLow: number, instanceIDHigh: number] => {return [0, '', 0, 0, 0]},
    GetLookAtLerpAmount: (): number => {return 0},
    GetMapInfo: (mapIndex: number): [teamSize: number, minLevel: number, maxLevel: number, numInstances: number] => {return [0, 0, 0, 0]},
    GetMatchDuration: (): number => {return 0},
    GetMaxNumPlayersPerTeam: (): number => {return 0},
    GetMaxNumTeams: (): number => {return 0},
    GetMode: (): number => {return 0},
    GetMsToHoldForHorizontalMovement: (): number => {return 0},
    GetMsToHoldForVerticalMovement: (): number => {return 0},
    GetMsToSmoothHorizontalChange: (): number => {return 0},
    GetMsToSmoothVerticalChange: (): number => {return 0},
    GetNumMaps: (): number => {return 0},
    GetNumPlayers: (factionIndex: number): number => {return 0},
    GetOrCreateSeries: (teamName1: string, teamName2: string): CommentatorSeries => {return {teams: {} as any}},
    GetPlayerAuraInfo: (teamIndex: number, playerIndex: number, spellID: number): [startTime: number, duration: number, enable: boolean] => {return [0, 0, false]},
    GetPlayerAuraInfoByUnit: (token: string, spellID: number): [startTime: number, duration: number, enable: boolean] => {return [0, 0, false]},
    GetPlayerCooldownInfo: (teamIndex: number, playerIndex: number, spellID: number): [startTime: number, duration: number, enable: boolean] => {return [0, 0, false]},
    GetPlayerCooldownInfoByUnit: (unitToken: string, spellID: number): [startTime: number, duration: number, enable: boolean] => {return [0, 0, false]},
    GetPlayerCrowdControlInfo: (teamIndex: number, playerIndex: number): [spellID: number, expiration: number, duration: number] => {return [0, 0, 0]},
    GetPlayerCrowdControlInfoByUnit: (token: string): [spellID: number, expiration: number, duration: number] => {return [0, 0, 0]},
    GetPlayerData: (teamIndex: number, playerIndex: number): CommentatorPlayerData | undefined => {return {unitToken: '', name: '', faction: 0, specialization: 0, damageDone: 0, damageTaken: 0, healingDone: 0, healingTaken: 0, kills: 0, deaths: 0}},
    GetPlayerFlagInfo: (teamIndex: number, playerIndex: number): boolean => {return false},
    GetPlayerFlagInfoByUnit: (unitToken: string): boolean => {return false},
    GetPlayerOverrideName: (originalName: string): string => {return ''},
    GetPlayerSpellCharges: (teamIndex: number, playerIndex: number, spellID: number): [charges: number, maxCharges: number, startTime: number, duration: number] => {return [0, 0, 0, 0]},
    GetPlayerSpellChargesByUnit: (unitToken: string, spellID: number): [charges: number, maxCharges: number, startTime: number, duration: number] => {return [0, 0, 0, 0]},
    GetPositionLerpAmount: (): number => {return 0},
    GetSmoothFollowTransitioning: (): boolean => {return false},
    GetSoftlockWeight: (): number => {return 0},
    GetSpeedFactor: (): number => {return 0},
    GetStartLocation: (mapID: number): Vector3DMixin => {return {} as any},
    GetTeamColor: (teamIndex: number): ColorMixin => {return {} as any},
    ...
    IsSmartCameraLocked: (): boolean => {return false},
    IsSpectating: (): boolean => {return false},
    IsTrackedDefensiveAura: (spellID: number): boolean => {return false},
    IsTrackedOffensiveAura: (spellID: number): boolean => {return false},
    IsTrackedSpell: (teamIndex: number, playerIndex: number, spellID: number, category: TrackedSpellCategory): boolean => {return false},
    IsTrackedSpellByUnit: (unitToken: string, spellID: number, category: TrackedSpellCategory): boolean => {return false},
    IsUsingSmartCamera: (): boolean => {return false},
    LookAtPlayer: (factionIndex: number, playerIndex: number, lookAtIndex: number | undefined): void => {},
    RemoveAllOverrideNames: (): void => {},
    RemovePlayerOverrideName: (originalPlayerName: string): void => {},
    RequestPlayerCooldownInfo: (teamIndex: number, playerIndex: number): void => {},
    ResetFoVTarget: (): void => {},
    ...
    SetDistanceBeforeForcedHorizontalConvergence: (distance: number): void => {},
    ...
    SetMapAndInstanceIndex: (mapIndex: number, instanceIndex: number): void => {},
    ...
    StartWargame: (listID: number, teamSize: number, tournamentRules: boolean, teamOneCaptain: string, teamTwoCaptain: string): void => {},
    SwapTeamSides: (): void => {},
    ToggleCheats: (): void => {},
...
export type CommentatorEnterWorldEvent = (frame: UIFrame, e: "COMMENTATOR_ENTER_WORLD") => void
export type CommentatorHistoryFlushedEvent = (frame: UIFrame, e: "COMMENTATOR_HISTORY_FLUSHED") => void
export type CommentatorImmediateFovUpdateEvent = (frame: UIFrame, e: "COMMENTATOR_IMMEDIATE_FOV_UPDATE", fov: number) => void
export type CommentatorMapUpdateEvent = (frame: UIFrame, e: "COMMENTATOR_MAP_UPDATE") => void
export type CommentatorPlayerNameOverrideUpdateEvent = (frame: UIFrame, e: "COMMENTATOR_PLAYER_NAME_OVERRIDE_UPDATE", nameToOverride: string, overrideName: string | undefined) => void
export type CommentatorPlayerUpdateEvent = (frame: UIFrame, e: "COMMENTATOR_PLAYER_UPDATE") => void
export type CommentatorResetSettingsEvent = (frame: UIFrame, e: "COMMENTATOR_RESET_SETTINGS") => void
export type CommentatorTeamNameUpdateEvent = (frame: UIFrame, e: "COMMENTATOR_TEAM_NAME_UPDATE", teamName: string) => void
export type CommentatorTeamsSwappedEvent = (frame: UIFrame, e: "COMMENTATOR_TEAMS_SWAPPED", swapped: boolean) => void